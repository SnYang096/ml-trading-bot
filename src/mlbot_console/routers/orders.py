...
def orders_trade_links(
    symbol: str = Query(..., description="Single symbol or * for all universe"),
    scopes: str = Query("trend,spot,multi_leg"),
    ...
    if is_all_symbols(symbol):
        ...
        all_links.sort(
            key=lambda r: int(r.get("exit_time") or r.get("entry_time") or 0),
            reverse=True,
        )
        out = all_links[: int(effective_limit)]
        return ok(out, meta={"count": len(out), "symbol": "ALL"})
