...
Moved from src/data_tools/feature_engineering_enhanced.py to src/strategies/time_series/features/
"""
...
class EnhancedFeatureEngineer:
    ...
    def _run_diagnostics(
        self, timeframe: str, df: pd.DataFrame, feature_cols: List[str]
    ) -> None:
        ...
        for col in feature_cols:
            ...
            for key, marker in clip_markers.items():
                metrics[key] = float(np.isclose(valid, marker, atol=tol_clip).mean())

            report[col] = metrics

            if (
                metrics["abs_ge_90pct_ratio"] > 0.05
                or metrics["abs_ge_99pct_ratio"] > 0.01
            ):
                flagged_clip.append(
                    (col, metrics["abs_ge_90pct_ratio"], metrics["abs_ge_99pct_ratio"])
                )
        ...
        messages: List[str] = []
        if flagged_clip:
            top_clip = sorted(flagged_clip, key=lambda x: x[1], reverse=True)[:5]
            clip_msg = f"   Possible clipping (abs>={near_clip:.2f}) " "detected:"
            details = ", ".join(
        ...
        if flagged_zero:
            top_zero = sorted(flagged_zero, key=lambda x: x[1], reverse=True)[:5]
            zero_msg = "   High zero saturation detected:"
            zero_details = ", ".join(f"{col}: {ratio:.1%}" for col, ratio in top_zero)
    ...
    def add_hilbert_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for source_name, source_data in signal_sources.items():
            ...
            source_series = source_series.replace([np.inf, -np.inf], np.nan).ffill()
            ...
            try:
                ...
                frequency_series = pd.Series(frequency[: len(df)], index=df.index)
                ...
                phase_unwrapped_series = pd.Series(phase_unwrapped, index=df.index)
    ...
    def add_advanced_derived_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            df["price_range_symmetry"] = (
                ...
                (
                    self._shift_feature(df["close"]) - self._shift_feature(df["low"])
                ).replace(0, np.nan)
            )
            df["price_range_symmetry"] = (
            ...
            up = (close_ref > self._shift_feature(df["close"], offset=1)).astype(int)
            ...
            df["acceleration_3"] = roc_3_shifted - self._shift_feature(roc_3, offset=1)

            # 12. Price vs EMA distance
            # Use configurable shift to avoid using current close (data leakage)
            df["price_vs_ema_distance"] = (
                self._shift_feature(df["close"]) - self._shift_feature(df["sma_20"])
            ...
            ).abs() / self._shift_feature(df["close"]).replace(0, np.nan)
            dist_low = (
                self._shift_feature(df["close"])
                - self._shift_feature(df["low"]).rolling(50).min()
            ).abs() / self._shift_feature(df["close"]).replace(0, np.nan)
    ...
    def add_order_flow_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if "buy_qty" in df.columns and "sell_qty" in df.columns:
                ...
                # Z-score标准化
                ofi_short_mean = ofi_short_raw.rolling(100, min_periods=20).mean()
                ofi_short_std = ofi_short_raw.rolling(100, min_periods=20).std()
                df["ofi_short"] = (
                    (
                        (ofi_short_raw - ofi_short_mean)
                        / ofi_short_std.replace(0, np.nan)
                    )
                ...
                ofi_medium_mean = ofi_medium_raw.rolling(100, min_periods=20).mean()
                ofi_medium_std = ofi_medium_raw.rolling(100, min_periods=20).std()
                df["ofi_medium"] = (
                    (
                        (ofi_medium_raw - ofi_medium_mean)
                        / ofi_medium_std.replace(0, np.nan)
                    )
                    .replace([np.inf, -np.inf], np.nan)
                    .fillna(0)
                    .clip(-self.feature_clip_bound, self.feature_clip_bound)
                ...
                df["cumulative_ofi"] = (
                    (
                        ...
                        / cumulative_ofi_std.replace(0, np.nan)
                    )
                    .replace([np.inf, -np.inf], np.nan)
                    .fillna(0)
                    .clip(-self.feature_clip_bound, self.feature_clip_bound)
                ...
                ofi_momentum_5_raw = df["order_flow_imbalance"].rolling(5).mean()
                ofi_momentum_20_raw = df["order_flow_imbalance"].rolling(20).mean()

                # Z-score标准化
                ofi_momentum_5_mean = ofi_momentum_5_raw.rolling(
                    50, min_periods=5
                ).mean()
                ofi_momentum_5_std = ofi_momentum_5_raw.rolling(50, min_periods=5).std()
                df["ofi_momentum_5"] = (
                    (
                        (ofi_momentum_5_raw - ofi_momentum_5_mean)
                        / ofi_momentum_5_std.replace(0, np.nan)
                    )
                    .replace([np.inf, -np.inf], np.nan)
                    .fillna(0)
                    .clip(-self.feature_clip_bound, self.feature_clip_bound)
                ...
                df["ofi_momentum_20"] = (
                    (
                        ...
                        / ofi_momentum_20_std.replace(0, np.nan)
                    )
                    .replace([np.inf, -np.inf], np.nan)
                    .fillna(0)
                    .clip(-self.feature_clip_bound, self.feature_clip_bound)
                ...
                ofi_volatility_raw = df["order_flow_imbalance"].rolling(20).std()
                ...
                df["ofi_volatility"] = (
                    (
                        ...
                        / ofi_volatility_std.replace(0, np.nan)
                    )
                    .replace([np.inf, -np.inf], np.nan)
                    .fillna(0)
                    .clip(-self.feature_clip_bound, self.feature_clip_bound)
            ...
            # 价格动量也需要标准化
            price_momentum_5_mean = price_momentum_5.rolling(50, min_periods=5).mean()
            price_momentum_5_std = price_momentum_5.rolling(50, min_periods=5).std()
            price_momentum_5_norm = (
                (
                    (price_momentum_5 - price_momentum_5_mean)
                    / price_momentum_5_std.replace(0, np.nan)
                )
                .replace([np.inf, -np.inf], np.nan)
                .fillna(0)
                .clip(-self.feature_clip_bound, self.feature_clip_bound)
            ...
            price_momentum_20_std = price_momentum_20.rolling(50, min_periods=20).std()
            price_momentum_20_norm = (
                (
                    (price_momentum_20 - price_momentum_20_mean)
                    / price_momentum_20_std.replace(0, np.nan)
                )
                .replace([np.inf, -np.inf], np.nan)
                .fillna(0)
                .clip(-self.feature_clip_bound, self.feature_clip_bound)
            ...
            if (
                ...
                df["cvd_short_trend"] = (
                    (
                        ...
                        / cvd_short_trend_std.replace(0, np.nan)
                    )
                    .replace([np.inf, -np.inf], np.nan)
                    .fillna(0)
                    .clip(-self.feature_clip_bound, self.feature_clip_bound)
                ...
                df["cvd_short_momentum"] = (
                    (
                        ...
                        / cvd_short_momentum_std.replace(0, np.nan)
                    )
                    .replace([np.inf, -np.inf], np.nan)
                    .fillna(0)
                    .clip(-self.feature_clip_bound, self.feature_clip_bound)
                ...
                df["cvd_medium_trend"] = (
                    (
                        ...
                        / cvd_medium_trend_std.replace(0, np.nan)
                    )
                    .replace([np.inf, -np.inf], np.nan)
                    .fillna(0)
                    .clip(-self.feature_clip_bound, self.feature_clip_bound)
                ...
                df["cvd_long_trend"] = (
                    (
                        ...
                        / cvd_long_trend_std.replace(0, np.nan)
                    )
                    .replace([np.inf, -np.inf], np.nan)
                    .fillna(0)
                    .clip(-self.feature_clip_bound, self.feature_clip_bound)
                ...
                cvd_short_medium_ratio_log = np.log1p(
                    np.abs(cvd_short_medium_ratio_raw)
                ) * np.sign(cvd_short_medium_ratio_raw)
                ...
                df["cvd_short_medium_ratio"] = (
                    (
                        ...
                        / cvd_short_medium_ratio_std.replace(0, np.nan)
                    )
                    .replace([np.inf, -np.inf], np.nan)
                    .fillna(0)
                    .clip(-self.feature_clip_bound, self.feature_clip_bound)
                ...
                cvd_medium_long_ratio_log = np.log1p(
                    np.abs(cvd_medium_long_ratio_raw)
                ) * np.sign(cvd_medium_long_ratio_raw)
                ...
                df["cvd_medium_long_ratio"] = (
                    (
                        ...
                        / cvd_medium_long_ratio_std.replace(0, np.nan)
                    )
                    .replace([np.inf, -np.inf], np.nan)
                    .fillna(0)
                    .clip(-self.feature_clip_bound, self.feature_clip_bound)
            ...
            # Z-score标准化
            cvd_slope_3_mean = cvd_slope_3_raw.rolling(50, min_periods=3).mean()
            cvd_slope_3_std = cvd_slope_3_raw.rolling(50, min_periods=3).std()
            df["cvd_slope_3"] = (
                (
                    (cvd_slope_3_raw - cvd_slope_3_mean)
                    / cvd_slope_3_std.replace(0, np.nan)
                )
                .replace([np.inf, -np.inf], np.nan)
                .fillna(0)
                .clip(-self.feature_clip_bound, self.feature_clip_bound)
            ...
            cvd_slope_10_mean = cvd_slope_10_raw.rolling(50, min_periods=10).mean()
            cvd_slope_10_std = cvd_slope_10_raw.rolling(50, min_periods=10).std()
            df["cvd_slope_10"] = (
                (
                    (cvd_slope_10_raw - cvd_slope_10_mean)
                    / cvd_slope_10_std.replace(0, np.nan)
                )
                .replace([np.inf, -np.inf], np.nan)
                .fillna(0)
                .clip(-self.feature_clip_bound, self.feature_clip_bound)
            ...
            cvd_slope_30_mean = cvd_slope_30_raw.rolling(50, min_periods=30).mean()
            cvd_slope_30_std = cvd_slope_30_raw.rolling(50, min_periods=30).std()
            df["cvd_slope_30"] = (
                (
                    (cvd_slope_30_raw - cvd_slope_30_mean)
                    / cvd_slope_30_std.replace(0, np.nan)
                )
                .replace([np.inf, -np.inf], np.nan)
                .fillna(0)
                .clip(-self.feature_clip_bound, self.feature_clip_bound)
            ...
            df["cvd_acceleration"] = (
                (
                    ...
                    / cvd_acceleration_std.replace(0, np.nan)
                )
                .replace([np.inf, -np.inf], np.nan)
                .fillna(0)
                .clip(-self.feature_clip_bound, self.feature_clip_bound)
            ...
            # 流动性比率，使用log转换和标准化
            liquidity_ratio_raw = df["volume"] / volume_ma_20.replace(0, np.nan)
            liquidity_ratio_log = np.log1p(liquidity_ratio_raw.fillna(1))
            liquidity_ratio_mean = liquidity_ratio_log.rolling(
                50, min_periods=20
            ).mean()
            liquidity_ratio_std = liquidity_ratio_log.rolling(50, min_periods=20).std()
            df["liquidity_ratio"] = (
                (
                    (liquidity_ratio_log - liquidity_ratio_mean)
                    / liquidity_ratio_std.replace(0, np.nan)
                )
                .replace([np.inf, -np.inf], np.nan)
                .fillna(0)
                .clip(-self.feature_clip_bound, self.feature_clip_bound)
            ...
            if "buy_qty" in df.columns and "sell_qty" in df.columns:
                ...
                buy_sell_pressure_ratio_raw = buy_sell_pressure_ratio_raw.fillna(1)
                buy_sell_pressure_ratio_log = np.log1p(buy_sell_pressure_ratio_raw)
                ...
                df["buy_sell_pressure_ratio"] = (
                    (
                        (buy_sell_pressure_ratio_log - buy_sell_pressure_ratio_mean)
                        / buy_sell_pressure_ratio_std.replace(0, np.nan)
                    )
                    .replace([np.inf, -np.inf], np.nan)
                    .fillna(0)
                    .clip(-self.feature_clip_bound, self.feature_clip_bound)
                ...
                pressure_diff_std = pressure_diff_raw.rolling(50, min_periods=20).std()
                df["pressure_diff"] = (
                    (
                        (pressure_diff_raw - pressure_diff_mean)
                        / pressure_diff_std.replace(0, np.nan)
                    )
                    .replace([np.inf, -np.inf], np.nan)
                    .fillna(0)
                    .clip(-self.feature_clip_bound, self.feature_clip_bound)
                ...
                df["pressure_diff_norm"] = df["pressure_diff"] / (
                    (buy_pressure_20 + sell_pressure_20).replace(0, np.nan) + 1e-10
                )
                df["pressure_diff_norm"] = (
                    df["pressure_diff_norm"].fillna(0).clip(-1, 1)
                )
    ...
    def add_basic_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        return BaselineFeatureEngineer.add_common_derived_features(
            data, rolling_zscore_windows=[50, 288, 500]
        )
    ...
    def _ensure_orderflow_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "taker_buy_ratio" not in out.columns:
            ...
            out["taker_buy_ratio"] = up.rolling(20, min_periods=5).mean().fillna(0.5)
        # Proxy CVD: cumulative signed volume using price change sign
        if "cvd" not in out.columns:
            signed_vol = np.sign(out["close"].diff().fillna(0.0)) * out[
                "volume"
            ].fillna(0.0)
            ...
            out["cvd_normalized"] = out["cvd_short"] / total_vol.replace(0, np.nan)
            out["cvd_normalized"] = (
                out["cvd_normalized"].replace([np.inf, -np.inf], np.nan).fillna(0.0)
            )
        return out
    ...
    def engineer_features(
        ...
        for timeframe, data in multi_tf_data.items():
            ...
            if self.enable_diagnostics:
                diag_cols = [col for col in feature_columns if col in df.columns]
