...
class BaselineFeatureEngineer:
    ...
    @staticmethod
    def compute_zigzag_high_low(
            zigzag: pd.Series) -> Tuple[pd.Series, pd.Series]:
        """
        从 ZigZag 序列中提取高点和低点
    ...
    def compute_poc(
        ...
        for i in range(window, len(high)):
            ...
            if total_volume > 0:
                poc_volume_ratio.iloc[
                    i] = bin_volumes[max_vol_idx] / total_volume
                ...
                while accumulated_volume < target_volume:
                    # 决定向上还是向下扩展
                    upper_vol = bin_volumes[upper_idx +
                                            1] if upper_idx + 1 < bins else 0
                    lower_vol = bin_volumes[lower_idx -
                                            1] if lower_idx - 1 >= 0 else 0
    ...
    def _compute_atr(df: pd.DataFrame, window: int = 14) -> pd.Series:
        ...
        return BaselineFeatureEngineer.compute_atr(df["high"],
                                                   df["low"],
                                                   df["close"],
                                                   period=window)
    ...
    def add_basic_indicators(
        ...
        if required_features is None or "rsi" in required_features:
            if "rsi" not in result.columns:
                result["rsi"] = BaselineFeatureEngineer.compute_rsi(
                    result["close"])
        ...
        if need_macd and "macd" not in result.columns:
            try:
                macd_line, signal_line, histogram = BaselineFeatureEngineer.compute_macd(
                    result["close"])
        ...
        need_bb = required_features is None or any(
            f in required_features
            for f in ["bb_upper", "bb_middle", "bb_lower"])
        if need_bb and "bb_upper" not in result.columns:
            try:
                upper_band, middle_band, lower_band = BaselineFeatureEngineer.compute_bollinger_bands(
                    result["close"])
        ...
        if required_features is None or "volatility" in required_features:
            if "volatility" not in result.columns:
                price_change_numeric = pd.to_numeric(
                    result["price_change"], errors="coerce").astype(float)
                values = talib.STDDEV(price_change_numeric.values,
                                      timeperiod=14,
                                      nbdev=1)
        ...
        if required_features is None or "volume_sma" in required_features or "volume_ratio" in required_features:
            if "volume_sma" not in result.columns:
                volume_numeric = pd.to_numeric(result["volume"],
                                               errors="coerce").astype(float)
                values = talib.SMA(volume_numeric.values, timeperiod=20)
                result["volume_sma"] = pd.Series(values,
                                                 index=volume_numeric.index)
    ...
    def ensure_basic_indicators(
        ...
        return BaselineFeatureEngineer.add_basic_indicators(
    ...
    def add_zigzag_dimensionless_features(
        ...
        # 提取 ZigZag 高点和低点
        zz_high, zz_low = BaselineFeatureEngineer.compute_zigzag_high_low(
            zigzag)
        ...
        if required_features is None or "price_to_zz_low_pct" in required_features:
            if "price_to_zz_low_pct" not in result.columns:
                result["price_to_zz_low_pct"] = ((close - zz_low) /
                                                 close).replace(
    ...
    def add_poc_hal_dimensionless_features(
            ...
            poc_window: int = 160) -> pd.DataFrame:
        """
        ...
        # 检查是否需要计算 POC 或 HAL
        need_poc = required_features is None or any("poc" in f
                                                    for f in required_features)
        need_hal = required_features is None or any("hal" in f
                                                    for f in required_features)
        ...
        need_compute = False
        if need_poc and ("poc" not in result.columns
                         or "poc_volume_ratio" not in result.columns):
            need_compute = True
        if need_hal and ("hal_high" not in result.columns
                         or "hal_low" not in result.columns):
            need_compute = True
        ...
        if required_features is None or "poc_position_ratio" in required_features:
            if "poc_position_ratio" not in result.columns:
                ...
                result["poc_position_ratio"] = ((poc - low) /
                                                price_range).replace(
                                                    [np.inf, -np.inf],
                                                    np.nan).clip(0.0, 1.0)
    ...
    def add_swing_dimensionless_features(
        ...
        if "roll_high_s" not in result.columns:
            if required_features and any("swing" in f
                                         for f in required_features):
                result["roll_high_s"] = result["high"].rolling(
                    swing_win_short, min_periods=1).max()
                result["roll_low_s"] = result["low"].rolling(
                    swing_win_short, min_periods=1).min()
                result["roll_high_l"] = result["high"].rolling(
                    swing_win_long, min_periods=1).max()
                result["roll_low_l"] = result["low"].rolling(
                    swing_win_long, min_periods=1).min()
            else:
                return result
        ...
        if required_features is None or "swing_high_pct_close" in required_features:
            if "swing_high_pct_close" not in result.columns:
                result["swing_high_pct_close"] = (
                    (result["roll_high_s"] - close) /
                    close.replace(0, np.nan)).replace([np.inf, -np.inf],
                                                      np.nan)
        ...
        if required_features is None or "swing_amplitude_pct" in required_features:
            if "swing_amplitude_pct" not in result.columns:
                ...
                result["swing_amplitude_pct"] = (
                    ...
                    roll_low_s_safe).replace([np.inf, -np.inf], np.nan)
    ...
    def add_price_volume_relative_features(
        ...
        if required_features is None or "rv_4h" in required_features:
            if "rv_4h" not in result.columns and "ret_1h" in result.columns:
                result["rv_4h"] = result["ret_1h"].rolling(
                    window=periods_4h // periods_1h, min_periods=1).std()
        ...
        if required_features is None or "vol_zscore" in required_features:
            if "vol_zscore" not in result.columns:
                vol_mean = volume.rolling(window=periods_24h,
                                          min_periods=1).mean()
                vol_std = volume.rolling(window=periods_24h,
                                         min_periods=1).std()
    ...
    def add_common_derived_features(
            ...
            if "volatility" not in result.columns:
                if "returns" in result.columns:
                    returns_numeric = pd.to_numeric(
                        result["returns"], errors="coerce").astype(float)
                    values = talib.STDDEV(returns_numeric.values,
                                          timeperiod=20,
                                          nbdev=1)
                    result["volatility"] = pd.Series(
                    ...
                    price_change_numeric = pd.to_numeric(
                        price_change, errors="coerce").astype(float)
                    values = talib.STDDEV(price_change_numeric.values,
                                          timeperiod=20,
                                          nbdev=1)
                    result["volatility"] = pd.Series(
        ...
        if {"bb_upper", "bb_lower"}.issubset(result.columns):
            ...
            if not required_features or "bb_width" in required_features:
                if "bb_width" not in result.columns:
                    ...
                    # 这样消除量纲，使其成为相对值
                    bb_width_raw = (result["bb_upper"] -
                                    result["bb_lower"]).abs()
                    if "bb_middle" in result.columns:
                        bb_middle_safe = result["bb_middle"].replace(0, np.nan)
                        result["bb_width"] = (bb_width_raw /
                                              bb_middle_safe).replace(
                                                  [np.inf, -np.inf], np.nan)
                    else:
        ...
        if not required_features or "macd_normalized" in required_features:
            if "macd_normalized" not in result.columns and "macd" in result.columns:
                result["macd_normalized"] = (result["macd"] /
        ...
        if not required_features or "volume_sma_20" in required_features:
            if "volume_sma_20" not in result.columns:
                ...
                result["volume_sma_20"] = pd.Series(
                    values,
                    index=volume_numeric.index).fillna(result["volume"])
    ...
    def _engineer_features_single_asset(
        ...
        if need_vwap:
            ...
            # 滚动求和：价格×成交量的滚动和
            rolling_pv = price_volume.rolling(window=self.vwap_window,
                                              min_periods=1).sum()
            # 滚动求和：成交量的滚动和
            rolling_vol = data["volume"].rolling(window=self.vwap_window,
                                                 min_periods=1).sum()

            # 滚动 VWAP = 滚动价格×成交量总和 / 滚动成交量总和
            vwap = (rolling_pv /
                    rolling_vol.replace(0, np.nan)).fillna(typical_price)
            ...
            # 价格相对 VWAP 的百分比偏离
            data["price_to_vwap_pct"] = ((data["close"] - vwap_safe) /
                                         vwap_safe).replace([np.inf, -np.inf],
                                                            np.nan)
            # 价格相对 VWAP 的 ATR 归一化偏离
            data["price_to_vwap_atr"] = ((data["close"] - vwap_safe) /
                                         (data["atr"] + eps)).replace(
                                             [np.inf, -np.inf], np.nan)
            # VWAP 偏离的 Z-score（相对于历史分布）
            vwap_deviation = (data["close"] - vwap_safe) / vwap_safe
            vwap_dev_mean = vwap_deviation.rolling(window=50,
                                                   min_periods=10).mean()
            vwap_dev_std = vwap_deviation.rolling(window=50,
                                                  min_periods=10).std()
            data["price_to_vwap_zscore"] = (
                (vwap_deviation - vwap_dev_mean) /
                vwap_dev_std.replace(0, np.nan)).replace([np.inf, -np.inf],
                                                         np.nan).fillna(0.0)
        ...
        if required_features is None or any(
            ...
            recent_rsi_high = data["rsi"].rolling(20,
                                                  min_periods=5).max().ffill()
            tol = 1e-8
            top_divergence_mask = (recent_high.notna()
            ...
            recent_rsi_low = data["rsi"].rolling(20,
                                                 min_periods=5).min().ffill()
            bottom_divergence_mask = (recent_low.notna()
                                      & recent_rsi_low.notna() &
                                      (data["close"] <= (recent_low + tol)) &
                                      (data["rsi"] > (recent_rsi_low + tol)))

            # 合并背离信号：顶背离=-1（看跌），底背离=+1（看涨），无背离=0
            data["rsi_divergence"] = (bottom_divergence_mask.astype(float) -
                                      top_divergence_mask.astype(float))

            # 成交量背离检测
            # 顶背离：价格上涨但成交量下降（看跌信号）
            price_vs_past_up = (data["close"]
            ...
            data["volume_divergence"] = (volume_bottom_div_mask.astype(float) -
        ...
        if need_compression:
            ...
            below = (data["atr_percentile"].fillna(0.5)
