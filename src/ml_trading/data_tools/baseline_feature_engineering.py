...
class BaselineFeatureEngineer:
    ...
    def engineer_features(self,
        ...
        if "hour_of_day_sin" not in data.columns:
            try:
                idx = data.index
                if hasattr(idx, "hour") and hasattr(idx, "dayofweek"):
                    data["hour_of_day_sin"] = np.sin(2 * np.pi * idx.hour / 24)
                    data["hour_of_day_cos"] = np.cos(2 * np.pi * idx.hour / 24)
                    data["day_of_week_sin"] = np.sin(2 * np.pi *
                                                     idx.dayofweek / 7)
                    data["day_of_week_cos"] = np.cos(2 * np.pi *
                                                     idx.dayofweek / 7)
                else:
                    data["hour_of_day_sin"] = 0
                    data["hour_of_day_cos"] = 1
                    data["day_of_week_sin"] = 0
                    data["day_of_week_cos"] = 1
            except Exception:
                data["hour_of_day_sin"] = 0
                data["hour_of_day_cos"] = 1
                data["day_of_week_sin"] = 0
        ...
        # Trend strength via rolling R² of close vs time index (short/long windows)
        index_series = pd.Series(np.arange(len(data)),
                                 index=data.index,
                                 dtype=float)
        r2_min_periods = 5
        data["trend_r2_short"] = data["close"].rolling(
            swing_win_short,
            min_periods=r2_min_periods).corr(index_series).pow(2)
        data["trend_r2_long"] = data["close"].rolling(
            swing_win_long,
            min_periods=r2_min_periods).corr(index_series).pow(2)
        data["trend_r2_short"] = data["trend_r2_short"].fillna(0.0)
        data["trend_r2_long"] = data["trend_r2_long"].fillna(0.0)
        ...
        feature_cols: List[str] = [
            ...
            # Trend strength (R² of linear trend)
            "trend_r2_short",
            "trend_r2_long",
            # Temporal features
            "hour_of_day_sin",
