...
class ClassificationStrategyHandler(BaseStrategyHandler):
    ...
    def generate_signals(
        ...
        # 2. Return regression: predicted return magnitude (can be positive or negative)
        return_pred = self.pipeline.return_models[timeframe].predict(X)
        ...
        signals_df = pd.DataFrame(
            {
                ...
                "return_pred": return_pred,  # Predicted return magnitude
        ...
        signals_df["confidence"] = np.abs(signals_df["class_proba"] - 0.5) * 2  # Scale to [0, 1]
        ...
        return_confidence = np.abs(signals_df["return_pred"]) / (signals_df["vol"] + 1e-8)
        signals_df["combined_confidence"] = signals_df["confidence"] * np.clip(return_confidence, 0, 1)
