...
def run_sr_reversal_rule_strategy(
    df: pd.DataFrame,
    atr: pd.Series,
    ...
    Args:
        df: 包含特征的DataFrame（应该已经包含vpin等特征）
        atr: ATR序列
        rule_params: 规则参数字典，包含：
            - stop_loss_r: 止损倍数（默认1.0）
            - take_profit_r: 止盈倍数（默认2.0）
            - max_holding_bars: 最大持仓周期（默认50）
            - atr_window: ATR窗口（默认14）
            - entry_offset: 入场偏移（默认1）
            - entry_price_col: 入场价格列（默认"open"）
            - use_breakeven_stop: 是否使用保本止损（默认False）
            - use_adaptive_rr: 是否使用自适应R/R（默认False）
            - volatility_window: 波动率窗口（默认10，仅在use_adaptive_rr=True时使用）
    
    Returns:
        包含以下字段的字典：
            - n_trades: 交易数量
            - win_rate: 胜率
            - breakeven_rate: 保本率（如果使用保本止损）
            - total_r: 总R值
            - sharpe_ratio: Sharpe比率
            - avg_r: 平均R值
    """
    # 提取参数，使用默认值
    stop_loss_r = float(rule_params.get("stop_loss_r", 1.0))
    take_profit_r = float(rule_params.get("take_profit_r", 2.0))
    max_holding_bars = int(rule_params.get("max_holding_bars", 50))
    atr_window = int(rule_params.get("atr_window", 14))
    entry_offset = int(rule_params.get("entry_offset", 1))
    entry_price_col = str(rule_params.get("entry_price_col", "open"))
    use_breakeven_stop = bool(rule_params.get("use_breakeven_stop", False))
    use_adaptive_rr = bool(rule_params.get("use_adaptive_rr", False))
    volatility_window = int(rule_params.get("volatility_window", 10))
    
    # 确保df有atr列，如果没有则使用传入的atr
    df_work = df.copy()
    if "atr" not in df_work.columns:
        df_work["atr"] = atr
    else:
        # 如果df中已有atr，确保索引对齐
        df_work["atr"] = atr.reindex(df_work.index, method="ffill")
    ...
    if use_adaptive_rr:
        ...
        labels_standard = compute_adaptive_rr_label_with_future_vol(
            ...
            stop_loss_multiplier=stop_loss_r,
            take_profit_multiplier=take_profit_r,
        ...
        if use_breakeven_stop:
            ...
            labels_breakeven = labels_standard
    else:
        # 标准版本：固定 R/R（基于 ATR）
    ...
    realized_r_standard = np.where(
        df_trades_standard["label"].values == 1.0, take_profit_r, -stop_loss_r
    )
    avg_r_standard = float(realized_r_standard.mean())
    total_r_standard = float(realized_r_standard.sum())
    
    # 计算Sharpe比率（基于R值序列）
    if len(realized_r_standard) > 1:
        sharpe_ratio = float(
            np.mean(realized_r_standard) / (np.std(realized_r_standard) + 1e-9) * np.sqrt(252)
        )
    else:
        sharpe_ratio = 0.0
    ...
    # 计算保本率
    breakeven_rate = 0.0
    if use_breakeven_stop and not use_adaptive_rr:
        # 尝试从details_breakeven获取保本信息
        if "final_result" in df_trades_breakeven.columns:
            n_breakeven_total = int(
                (df_trades_breakeven["final_result"].isin(["breakeven_win", "breakeven_loss"])).sum()
            )
            breakeven_rate = (
                n_breakeven_total / n_trades_breakeven if n_trades_breakeven > 0 else 0.0
            )
    
    # 返回结果（使用标准版本的结果作为主要指标）
    return {
        "n_trades": n_trades_standard,
        "win_rate": win_rate_standard,
        "breakeven_rate": breakeven_rate,
        "total_r": total_r_standard,
        "sharpe_ratio": sharpe_ratio,
        "avg_r": avg_r_standard,
    }
