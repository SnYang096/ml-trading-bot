...
def load_portfolio_assets_config(path: str | Path) -> PortfolioAssetsConfig:
    p = Path(path)
    obj = yaml.safe_load(p.read_text(encoding="utf-8")) or {}
    return PortfolioAssetsConfig(
        name=str(obj.get("name", "portfolio_assets")),
...
def compute_portfolio_asset_weights(
    *,
    cfg: PortfolioAssetsConfig,
    sig: RouterAggregateSignals,
