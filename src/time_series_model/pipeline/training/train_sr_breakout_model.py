...
def main():
    parser = argparse.ArgumentParser(description="Train SR Breakout Model")
    parser.add_argument(
        "--data-path",
        type=str,
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default="results/sr_breakout_model",
    ...
    print("=" * 60)
    print("📈 SR Breakout Model Training")
    print("=" * 60)
    ...
    breakout_features = select_breakout_features(df_train_features, feature_cols)
    print(
        f"\n   📊 Selected {len(breakout_features)} breakout features (from {len(feature_cols)} total)"
    )
    ...
    results_file = output_dir / "breakout_results.json"
    ...
    print(f"  Train Samples: {len(df_train_breakout)}")
    print(f"  Test Samples: {len(df_test_breakout)}")
    print("=" * 60)
