...
from src.features.time_series.baseline_features import engineer_baseline_features
...
def safe_multi_asset_preprocessing(
    ...
    Note:
        ...
        - future_volatility 使用滚动 RMS 代理，避免未来信息泄露
    ...
    for symbol, symbol_file_list in symbol_files.items():
        ...
        try:
            ...
            symbol_resampled = _resample_single_asset(
                symbol_data_for_resample, timeframe
            )
            ...
            if "symbol" not in symbol_resampled.columns:
                raise ValueError(
                    f"❌ 严重错误：{symbol} 的 resampled 数据缺少 symbol 列！"
        ...
        symbol_col_backup = (
            symbol_for_features["symbol"].copy()
            if "symbol" in symbol_for_features.columns
            else None
        )
            ...
            for col in symbol_feat_df.columns:
                if col in raw_cols_to_exclude:
                            ...
                            if len(col_values) > 0:
                                ...
                                if col_abs_max > 1000:
                                    ...
                                    col_values_scaled = scaler.fit_transform(
                                        col_values.values.reshape(-1, 1)
                                    ).flatten()
                                    symbol_feat_df.loc[col_values.index, col] = (
                                        col_values_scaled
                                    )
                                    print(
                    ...
                    # 对于 roll_high_* 和 roll_low_*，直接排除（已有标准化版本 sr_dist_*）
                        ...
                        if (
                            col == "roll_high_s"
                            and "sr_dist_high_s" in symbol_feat_df.columns
                        ):
                            cols_to_drop.append(col)
                            print(
                            ...
                            col == "roll_low_s"
                            and "sr_dist_low_s" in symbol_feat_df.columns
                        ):
                            cols_to_drop.append(col)
                            print(
                            ...
                            col == "roll_high_l"
                            and "sr_dist_high_l" in symbol_feat_df.columns
                        ):
                            cols_to_drop.append(col)
                            print(
                            ...
                            col == "roll_low_l"
                            and "sr_dist_low_l" in symbol_feat_df.columns
                        ):
                            cols_to_drop.append(col)
                            print(
        ...
        print(
            f"   📈 计算标签 {symbol}（使用完整数据 {len(symbol_resampled):,} 条）..."
        )
        ...
        if len(close_prices) > 0:
            ...
            if extreme_jumps > 0:
                print(
                    f"         ⚠️  警告: 检测到 {extreme_jumps} 个极端价格跳空（>50%）"
                )
        ...
        try:
            ...
            if len(simple_returns_valid) > 0:
                ...
                # 检查极端值
                extreme_simple = (simple_returns_valid.abs() > 1.0).sum()  # >100%
                if extreme_simple > 0:
                    extreme_ratio = extreme_simple / len(simple_returns_valid) * 100
                    print(
            ...
            if len(valid_log_returns_clean_diag) > 0:
                ...
                print(f"         标准差: {valid_log_returns_clean_diag.std():.6f}")
                # 检查是否有 Inf 或极端值
                ...
                extreme_log = (
                    valid_log_returns_clean_diag.abs() > 2.0
                ).sum()  # log(3) ≈ 1.1, log(10) ≈ 2.3
                if extreme_log > 0:
                    extreme_ratio = (
                        extreme_log / len(valid_log_returns_clean_diag) * 100
                    )
                    print(
            ...
            if not valid_log_returns.index.equals(symbol_feat_df.index):
                ...
                valid_log_returns = valid_log_returns.reindex(symbol_feat_df.index)
            ...
            if len(valid_log_returns_clean) > 0:
                ...
                try:
                    ...
                    winsorized_values = np.clip(
                        winsorized_values, -max_log_return, max_log_return
                    )
                    ...
                    future_return_series = pd.Series(
                        future_return_values, index=valid_log_returns_clean.index
                    )

                    # 确保索引对齐（使用 symbol_feat_df 的索引）
                    symbol_feat_df["future_return"] = future_return_series.reindex(
                        symbol_feat_df.index, fill_value=np.nan
                    )

                    # 🔧 OPTIMIZATION: 验证 future_return 的合理性
                    future_return_stats = symbol_feat_df["future_return"].dropna()
                    if len(future_return_stats) > 0:
                        ...
                        if extreme_count > 0:
                            extreme_ratio = (
                                extreme_count / len(future_return_stats) * 100
                            )
                            print(
                    ...
                    future_return_final = symbol_feat_df["future_return"].dropna()
                    if len(future_return_final) > 0:
                        ...
                        # 检查是否还有极端值
                        extreme_final = (future_return_final.abs() > 1.0).sum()  # >100%
                        if extreme_final > 0:
                            extreme_ratio = (
                                extreme_final / len(future_return_final) * 100
                            )
                            print(
                    ...
                    future_return_values = np.exp(valid_log_returns_clean.values) - 1
                    future_return_series = pd.Series(
                        future_return_values, index=valid_log_returns_clean.index
                    )
                    symbol_feat_df["future_return"] = future_return_series.reindex(
        ...
        if symbol_col is not None and "symbol" not in symbol_feat_df.columns:
            symbol_feat_df["symbol"] = symbol_col.reindex(
                symbol_feat_df.index, fill_value=symbol
            )
        ...
        if "symbol" not in symbol_feat_df.columns:
            raise ValueError(
                f"❌ 严重错误：{symbol} 的最终数据缺少 symbol 列！"
                "这会导致多标的合并不安全。"
            )
        if not (symbol_feat_df["symbol"] == symbol).all():
        ...
        # 验证 symbol 列
        if "symbol" in symbol_feat_df.columns:
            print(f"         ✅ Symbol 列验证通过: {symbol_feat_df['symbol'].unique()}")
    ...
    print(f"{'='*70}")
    print(f"🔗 合并所有标的的数据...")
    final_df = pd.concat(all_processed_dfs, axis=0, ignore_index=False).sort_index()
    print(f"   最终数据量: {len(final_df):,} 条")

    # 🔒 CRITICAL: 验证合并后的数据必须包含 symbol 列
    if "symbol" not in final_df.columns:
        raise ValueError(
            "❌ 严重错误：合并后的数据缺少 symbol 列！"
    ...
    if "UNKNOWN" in final_df["symbol"].unique():
        ...
        print(
            f"      💡 建议：检查数据文件，确保所有文件都包含 symbol 列或文件名包含标的标识"
        )
        if unknown_ratio > 10:
            print(
                f"      🚨 严重警告：UNKNOWN 样本占比过高（>10%），建议停止训练并修复数据"
            )
            raise ValueError(
                f"UNKNOWN symbol 样本占比过高（{unknown_ratio:.2f}%），"
    ...
    if "symbol" in final_df.columns and "future_return" in final_df.columns:
        for symbol in final_df["symbol"].unique():
            ...
            if len(symbol_fr) > 0:
                ...
                print(f"         范围: [{symbol_fr.min():.6f}, {symbol_fr.max():.6f}]")
                print(
        ...
        if len(fr_stats_by_symbol) > 1:
            ...
            abs_maxs = [stats["abs_max"] for stats in fr_stats_by_symbol.values()]
