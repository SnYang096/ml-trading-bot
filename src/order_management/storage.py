...
class Storage:
    ...
    def get_safety_state(self, *, state_id: str = "global") -> Optional[Dict[str, Any]]:
        ...
        try:
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM safety_state WHERE state_id = ?", (state_id,))
            row = cursor.fetchone()
            if not row:
                return None
        ...
        try:
            ...
            """,
                (
                    ...
                    (
                        json.dumps(position.trailing_stop_config)
    ...
    def get_latest_add_entry_time(self, *, parent_position_id: str) -> Optional[str]:
        """Return latest add-leg entry_time for a parent position id.

        Uses ``positions.parent_position_id`` written by live add-position path.
        Returns raw timestamp string as stored in SQLite (or None if unavailable).
        """
        pid = str(parent_position_id or "").strip()
        if not pid:
            return None
        conn = self._get_connection()
        try:
            ...
            if not row:
                return None
            value = row["entry_time"] if isinstance(row, sqlite3.Row) else row[0]
    ...
    def _row_to_position(self, row: sqlite3.Row) -> Position:
        ...
        return Position(
            ...
            exit_time=(
                ...
                if row["exit_time"] and isinstance(row["exit_time"], str)
                else row["exit_time"]
            ),
            ...
            created_at=(
                ...
                if row["created_at"] and isinstance(row["created_at"], str)
                else row["created_at"]
            ),
            updated_at=(
    ...
    def _row_to_order(self, row: sqlite3.Row) -> Order:
        ...
        return Order(
            ...
            client_order_id=(
                row["client_order_id"] if "client_order_id" in row.keys() else None
            ),
            position_id=row["position_id"],
            symbol=row["symbol"],
            ...
            filled_at=(
                ...
                if row["filled_at"] and isinstance(row["filled_at"], str)
                else row["filled_at"]
            ),
            canceled_at=(
    ...
    def _row_to_position_operation(self, row: sqlite3.Row) -> PositionOperation:
        ...
        return PositionOperation(
            ...
            operation_time=(
                datetime.fromisoformat(row["operation_time"])
                if isinstance(row["operation_time"], str)
                else row["operation_time"]
            ),
            size=row["size"],
            price=row["price"],
            pnl=row["pnl"],
    ...
    def get_stop_loss_trailing_history(
        self, position_id: str
    ) -> List[StopLossTrailing]:
        """获取仓位的止损上移历史"""
        conn = self._get_connection()
        try:
            cursor = conn.cursor()
    ...
    def _row_to_stop_loss_trailing(self, row: sqlite3.Row) -> StopLossTrailing:
        ...
        return StopLossTrailing(
            ...
            move_time=(
                datetime.fromisoformat(row["move_time"])
                if isinstance(row["move_time"], str)
                else row["move_time"]
            ),
        ...
        try:
            ...
            """,
                (
                    ...
                    (
                        metrics.date.date()
                        if isinstance(metrics.date, datetime)
                        else metrics.date
                    ),
                    metrics.symbol,
    ...
    def get_performance_metrics(
        self, date: Optional[datetime] = None, symbol: Optional[str] = None
        ...
        try:
            ...
            if date and symbol:
                ...
                cursor.execute(
                    "SELECT * FROM performance_metrics WHERE symbol = ?", (symbol,)
                )
            else:
