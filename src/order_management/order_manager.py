...
class OrderManager:
    ...
    def place_order(
        ...
        with self._lock:
            ...
            order = Order(
                order_id=order_id,
                binance_order_id=str(binance_order['order_id']),
                position_id=position_id,
                symbol=symbol,
                ...
                price=price,
                stop_price=stop_price,
                status=self._convert_order_status(binance_order['status']),
            ...
            if self.storage.create_order(order):
                logger.info(f"下单成功: {order_id}, {symbol}, {side.value}, {order_type.value}")
                return order
            else:
        ...
        with self._lock:
            ...
            try:
                success = self.binance_api.cancel_order(
                    order.binance_order_id or order_id,
                    order.symbol
                )
            except Exception as e:
