...
6) Quality comparison (original features vs compressed/Top-K)
   ...
   - Production-style comparison (single command; trains both and writes a summary JSON). Supports optional date window:
   ```bash
   make dim-compare SYMBOL=BTCUSDT ENCODING_DIM=16 \
     START_DATE=2024-10-01 END_DATE=2024-12-31 \
     DIM_COMPARE_ARGS="--top-k 40"
   ```
   - Alternative (manual two-run): train full with `make train`, then train Top-K with `make train-topk` using the Top-K file from `make dimensionality-real`.

Differences: `production_training` (used by `make dim-compare`) runs a paired evaluation under one script, ensuring identical data splits and producing a direct metrics summary (R²/RMSE/MAE) for original vs compressed; the two-run method gives you two standalone artefacts you can also backtest separately.
