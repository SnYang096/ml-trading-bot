...
### Phase 1 完成 (2026-01-01)

| 特征 | 归一化方式 | 输出范围 | 状态 |
|------|----------|---------|------|
| `atr_f` | `atr / close` | [0.001, 0.1] | ✅ 完成 |
| `macd_f` | `macd / ATR` | [-3, 3] | ✅ 完成 |
| `bb_width_f` | `bb_width_normalized`, `bb_position` | [0, 1] | ✅ 完成 |
| `poc_hal_features_f` | `(level - close) / ATR` | [-5, 5] | ✅ 完成 |
| `sr_strength_max_f` | `dist / ATR` | [-3, 3] | ✅ 完成 |

### 树模型 vs NN 模型

| 模型类型 | 归一化要求 | 说明 |
|---------|-----------|------|
| 树模型 (LightGBM) | 可选 | 树模型对尺度不敏感，但归一化有助于跨资产泛化 |
| NN 模型 (multihead) | 必需 | 神经网络需要归一化输入，否则训练不稳定 |

### 参考文档

- [FEATURE_NORMALIZATION_POLICY.md](../architecture/FEATURE_NORMALIZATION_POLICY.md) - 归一化策略详细说明
