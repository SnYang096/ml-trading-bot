...
│    → 特征计算（列集合随 archetypes 汇总）                            │
│    → decision_handler.decide(features)（GenericLiveStrategy）        │
│    → 持仓状态机（以配置为准）                                        │
└───────────────────────┬─────────────────────────────────────────────┘
                        │ features
                        ▼
┌─────────────────────────────────────────────────────────────────────┐
│         GenericLiveStrategy（BPC：无分档 sizing）                      │
│    Prefilter → Direction → Gate → Entry → Execution → TradeIntent    │
│    → 无 ML；不做分档验收                                              │
└───────────────────────┬─────────────────────────────────────────────┘
                        │ TradeIntent
                        ▼
...
| 文件 | 职责 |
|------|------|
| `scripts/run_live.py` | 统一实盘入口；装配 **GenericLiveStrategy** + LivePCM |
| `src/time_series_model/live/generic_live_strategy.py` | **配置驱动决策引擎**（Prefilter→…→Execution） |
| `src/live_data_stream/order_flow_listener.py` | Tick 聚合 + 特征计算 + 决策路由 + 持仓状态机 |
