...
Layer4  Execution              单仓生命周期管理（entry/exit/SL/TP 机械逻辑）
Layer5  PCM_Portfolio          slot rotation / add-on / 风险预算 / Sharpe唯一观测层
...
- **PCM/Portfolio** 是唯一允许看 Sharpe 的层，负责 slot rotation / add-on 许可 / 风险预算
...
flowchart TD
  ...
  Gate --> PCM[PCM: Slot Rotation / Add-on / Risk Budget]
  PCM --> Exec[ExecutionPolicies]
