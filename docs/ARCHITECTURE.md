...
flowchart TD
  ...
  Materialize --> FeaturePlan[feature_plan_v1.yaml]
  Materialize --> ModelCfg[train/predict configs]
  Materialize --> StratProfiles[strategies/*/profile.yaml]
  StratProfiles --> ExecRegistry[execution_archetypes_v1.yaml]
  ...
  ModelCfg --> Router[RuleRouter 3-action]
  Router --> Gate[Tree Gate / Veto / Bias]
  Gate --> LiveExec
  Gate --> Backtest
```
...
配置位置：`config/nnmultihead/execution_archetypes_v2.yaml`
...
- **与 Router 解耦**：Router 选模式，evidence 只做 gate/bias
