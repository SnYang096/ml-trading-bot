...
与「Nautilus 作边缘 Adapter、主干自研」的取向一致（原 `live_stream` 下相关短文已移除；历史讨论见 `docs/archive/live_stream/legacy/`）：  
...
- **事件回测与实盘对齐**：`docs/architecture/event_drive_backtest/`（替代已移除的 Nautilus 对齐清单类文档）
