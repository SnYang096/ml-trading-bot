...
- `docs/archive/architecture/谁对sharp负责.md`
- `docs/archive/architecture/极端情况暴富和生存的问题.md`
- `docs/architecture/树模型在多头模型下游的角色.md`
- `docs/architecture/多头NN和订单流.md`
- `docs/archive/architecture/赌大人物meme的本质或者消息的本质.md`

另：特征分层与研发闭环参考：
- `docs/architecture/guides/FEATURE_COMPLEXITY_LAYERS_CN.md`
- `docs/archive/guides/tree/DEPLOYMENT_MVP_WORKFLOW_CN.md`
- Pool‑B 反向特征说明：`docs/archive/guides/tree/POOLB_INVERT_FEATURES_CN.md`
...
- **示例 gate 配置**：`config/kpi_gates/router_counterfactual.yaml`
...
flowchart TD
  ...
  modelLayer --> routerLayer[Layer3_WorldRegimeArchetype]
  routerLayer --> gateLayer[Layer3b_TreeGateDetector]
  gateLayer --> execLayer[Layer4_Execution]
  execLayer --> pcmLayer[Layer5_PCM_CapitalPolicy]
...
- **Layer 3 World/Regime/Archetype**：只负责物理可行性/状态/行为模板的划分，不输出收益结论。  
- **Layer 3b Tree Gate/Detector**：只负责 veto/throttle/permission（不直接输出方向、不直接下单）。  
- **Layer 4 Execution**：只负责“按预算执行、成本/滑点/风控一致性”。  
- **Layer 5 PCM（Portfolio Capital Manager）**：**唯一用 Sharpe 做生产决策** 的层；负责把“可交易资金/风险预算”按状态与约束分配出去。  
...
（以下是 schema 设计意图，具体 YAML 见 `task_spec.yaml`）
...
（具体字段见 `config/constitution/constitution.yaml`）
...
- **唯一允许用 Sharpe 做生产决策的地方**：组合层的 Sharpe/DD/相关性/尾部风险
...
这部分请以两份文档为主：
- 特征/Universe 分层：`docs/architecture/guides/FEATURE_COMPLEXITY_LAYERS_CN.md`
- 上线 MVP 闭环：`docs/archive/guides/tree/DEPLOYMENT_MVP_WORKFLOW_CN.md`
- 命令导向最短路径：`docs/architecture/guides/RD_TO_LIVE_TIERED_WORKFLOW_V1_CN.md`

---

## 6) 实盘（Live）部分：把 `docs/architecture/live_stream` 当作“边缘系统”而不是系统核心
...
这与 `docs/architecture/live_stream/nautilus_trader当作边缘系统.md` 的结论一致：  
...
- **入口索引**：`docs/architecture/live_stream/README.md`
- **一致性原则与契约**：`docs/architecture/live_stream/01_一致性原则与契约.md`
- **事件流与时间对齐**：`docs/architecture/live_stream/02_事件流与时间对齐.md`
- **特征计算：状态与缓存**：`docs/architecture/live_stream/03_特征计算_状态与缓存.md`
- **存储：回放与审计**：`docs/architecture/live_stream/04_存储_回放与审计.md`
- **补全/对账/异常处理**：`docs/architecture/live_stream/05_补全_对账与异常处理.md`
- **实盘稳定性运行手册**：`docs/architecture/live_stream/06_实盘稳定性运行手册.md`
- **与 NautilusTrader 对齐清单**：`docs/architecture/live_stream/07_与NautilusTrader对齐清单.md`
- **Nautilus 集成指南**：`docs/architecture/live_stream/reference/Nautilus_Trader_集成指南.md`
...
- **任何 promotion/上线动作必须过 KPI Gate（hard-fail）**：  
  ...
  - gate 配置示例：`config/kpi_gates/router_counterfactual.yaml`
