# 如何选择 dim-compare 结果用于 Rolling 训练和回测

## 问题回答

### 1. Rolling train 是否都可以加载 dim-compare 输出的 top factors？

**答案：是的！** 所有 rolling train 命令都支持通过 `--use-top-factors` 参数加载 dim-compare 输出的 `top_factors.json`。
...
### 2. 应该用哪个 dim-compare 结果去训练生产模型进行回测？

## 选择原则

### 原则 1: 时间范围匹配

选择与你的**训练/回测时间范围最接近**的 dim-compare 结果：
...
**最佳实践**：选择**覆盖时间范围最长**或**最新**的 dim-compare 结果，因为：
- 更长的历史数据能更好地识别稳定特征
- 最新的结果反映了当前市场环境

### 原则 2: 特征类型一致

确保 dim-compare 使用的 `--feature-type` 与训练时一致：

```bash
# dim-compare 使用 comprehensive
make dim-compare DIM_COMPARE_FEATURE_TYPE=comprehensive ...
...
确保 dim-compare 的 `TIMEFRAME` 和 `HORIZONS` 与训练配置一致：

```bash
# 示例：dim-compare 使用 60T 时间框架，forward bars=5
# 目录名包含：tf60T_h5
...
### 方案 A: 使用最新最全面的 dim-compare 结果（推荐）

```bash
# 1. 运行 dim-compare（使用较长的时间窗口，如最近 2-3 年）
...
### 方案 B: 使用多个时间窗口的 dim-compare 结果（更稳健）

如果你有多个时间窗口的 dim-compare 结果，可以：
...
```bash
# 假设你已经运行了：
# make dim-compare SYMBOLS=BTCUSDT,ETHUSDT START_DATE=2024-01-01 END_DATE=2024-12-31 ...
...
在选择 dim-compare 结果前，确认：

- [ ] **时间范围匹配**：dim-compare 的时间范围覆盖或接近训练/回测时间
- [ ] **特征类型一致**：`DIM_COMPARE_FEATURE_TYPE` = `ROLLING_FEATURE_TYPE`
- [ ] **时间框架一致**：dim-compare 的 `TIMEFRAME` = rolling 的 `ROLLING_FREQ`
- [ ] **Forward bars 一致**：dim-compare 的 `HORIZONS` = rolling 的 `ROLLING_FBS`
- [ ] **性能指标良好**：查看 `production_results.json` 确认性能可接受
- [ ] **文件存在**：确认 `top_factors.json` 文件存在且可读

## 常见问题

### Q: 如果找不到完全匹配的 dim-compare 结果怎么办？
...
A: **不推荐**。不同时间框架的特征重要性可能不同，应该为每个时间框架运行独立的 dim-compare。

### Q: 多久需要重新运行 dim-compare？

A: 建议：
- **季度更新**：每季度运行一次 dim-compare，使用最近 1-2 年的数据
- **市场变化时**：当市场环境发生重大变化时（如牛熊转换）
- **性能下降时**：当模型性能明显下降时

### Q: 多个 dim-compare 结果可以混合使用吗？
