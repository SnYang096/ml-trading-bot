...
- **PCM candidates**：按 constitution 与 pipeline 配置严格对齐后生成的趋势 / multi-leg 候选池
...
**滚测入口**：`fast_month` / `rolling_sim` 等阶段要求 **管线声明的 `strategies` 为宪法的子集**：经典名 ⊆ `enabled_archetypes`，multi-leg 名 ⊆ `multi_leg.strategies`。宪法可列出更多实盘允许名；只跑 `bpc` 的 `slow.yaml` 不必删宪法里的 `tpc`/`me`。

**PCM 联合事件回测**：当月实际送入联合回测的策略集，仍以 **管线候选人 ∩ 宪法 allowlist** 为准（条数 ≥2 才跑）。
...
- `mlbot pipeline report-side-state --run-id <run_id>`：扫描 **`pcm_candidates_*.json`**，按月列出 `trend_pcm_candidate` 与 `multi_leg_pcm_candidate` 两个候选池。**不再**读取 `symbol_side_state.json`。
- `mlbot pipeline debug-pcm-candidates --run-id <run_id> --month YYYY-MM`
...
- `fast_month_<YYYY-MM>/`
  ...
  - `pcm_candidates_<YYYY-MM>.json`（逐策略 **`metrics`**、`trend_pcm_candidate` / `multi_leg_pcm_candidate` 等）
  - `multi_leg_pcm_<YYYY-MM>.json`（multi-leg 独立池诊断；含同 symbol 冲突检查）

## 6. PCM 候选契约

PCM 候选不再计算或配置质量分。当前规则只有两条：策略必须同时存在于 pipeline 与 constitution；通过当月事件回测后，趋势策略进入 `trend` 池，多腿策略进入 `multi_leg` 池。

## 7. 策略上下线 / 月度「记忆」（已移除）

原 **Symbol Side 状态机**（`symbol_policy`、`active/carry_forward/disabled`、`symbol_side_state.json`、跨月 `prev_side_state`）已从 `rolling_sim` / `fast_month` 移除：策略是否参与研究/上线改由 **配置文件与 constitution（及 slow 场景的 `adoption_gate`）** 显式控制，不做自动下线。**PCM**：trend 与 multi-leg 为独立账户池；trend 全量候选进入联合回测，multi-leg 走独立池且 enforce「同 symbol 同时最多一个 multi-leg 策略」。**不靠**月度分数或人数封顶删减名单。
...
- 热更新：阈值、execution 等
- 需重启：特征集合与 constitution 硬约束、策略是否在配置中启用
- retrain 检查仍由现有周期任务触发，rolling 结果作为决策依据

## 9. 验证计划

1. **稳定性**：既有训练、校准与事件回测流程输出不变
2. **功能性**：新阶段产物齐全、CLI 可调用
3. **一致性**：`monthly_ledger` 与 `stitched_summary` 聚合一致
4. **可复现**：固定 seed/date 结果稳定
5. **PCM 覆盖面**：配置的 archetype 数 ≥2 时联合回测是否正常触发。

## 10. 风险与后续

- V1 风险：若管线配置了宪法未授权的 archetype 会直接失败；宪法侧多出的条目用于实盘 / 编排，不要求单策略研究与宪法列表逐字相等。
  - PCM 仍分 trend / multi-leg 两池；联合回测用管线候选项与宪法白名单共同约束。
- V1.1 方向：
