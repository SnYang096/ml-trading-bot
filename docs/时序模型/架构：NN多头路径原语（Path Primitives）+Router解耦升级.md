...
- **通用底座 ≠ 全量特征默认通用**。  
  底座应默认只吃“结构稳健特征”，争议特征必须可控（可开关/可降权/可隔离）。
- **不要为每个策略训练一套底座**（回到旧范式：样本浪费、维护爆炸、Router 复用价值下降）。  
  真需要“版本化”，也应该是 **少量（2–4）**、按 market_profile/regime 的底座版本，而不是按策略名拆。

特征分层（建议写进 `config/nnmultihead/*/features.yaml` 的治理规范）：

- **A. 核心通用组（默认 on）**：kline/波动/趋势强度/压缩度/SR 距离等（更接近“路径几何”的稳定解释变量）。
- **B. 辅助确认组（默认 on，但必须切片监控）**：例如部分 `CVD` 类（在反转任务里接近中性，更多用于“确认/过滤”而非主驱动）。
- **C. 争议组（默认 off，必须先做分组消融）**：例如 `TradeCluster/VPIN` 一类。  
  这类组的进入条件必须是：在你关心的 slice 上（如 `near_sr`/`high_trendiness`）**明确正贡献**，否则保持 off。
- **D. Execution 专用组（不进底座；只给 Execution/风控/摩擦模型）**：滑点/成交率/清算距离/funding/latency 等。  
  原因：它们属于“执行摩擦/安全壳”，不应污染“路径原语”学习目标。

落地方式（从稳到进阶，按阶段选）：

- **Level 0（最稳，推荐起步）配置门控**：训练/推理时按配置选择特征组（on/off），并且把“组定义”冻结进版本。  
  - 优点：可审计、可复现、和你现有“分组消融协议”完全一致。  
  - 风险：配置组合爆炸 ⇒ **严格限制组数 < 5**，并限制“可选组”数量。
- **Level 1（进阶）场景门控**：Router/监控先识别场景（如 `trendiness/compression/liquidity/jump` bucket），再决定争议组是否启用。  
  - 这是“路由分工”的自然延伸：**Router 管场景，底座管路径**。
- **Level 2（高级）Learned gating / adapter / MoE**：底座共享 backbone，但对争议组用小 adapter 或稀疏专家路由做隔离。  
  - 只有在样本显著变多、且你已经把 Level0/1 跑稳后再做（否则调参成本会吞掉收益）。
