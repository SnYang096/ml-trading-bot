...
- **`pipeline list` 只扫一个 `output.history_dir`**：实现上 `_cmd_list_experiments(history_dir, strategy)` 只读 **当前加载 pipeline 配置里的** `history_dir / <strategy>`（见 `scripts/auto_research_pipeline.py`）。因此 **换一份 `--config`（例如默认 `research_pipeline.yaml` vs `config/strategies/chop_grid/research/calibrate_roll.default.yaml`）就会「看不到」** `results/chop_grid/turbo-rolling-sim`、`results/dual_add_trend/turbo-rolling-sim` 等目录下的时间戳——**不是 bug，而是单根设计**。要「看到所有研究成果」，需要 **新命令或索引层**（见 §11.4），静态站点是合理延伸（扫描多根下的 `report.json` / `report.html` / `trading_map*.html` / `stitched_summary.json` 等）。
