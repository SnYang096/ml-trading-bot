...
| **deploy** | **默认只**将 `config/strategies/<slug>/archetypes/`（+ 脚本约定的根上少量文件，如多腿 `grid.yaml`/`dual_add.yaml`）同步到 `live/highcap/config/strategies/<slug>/`。 |
...
以 `bpc` 为例（`tpc`/`me` 同构；多腿根文件名为 `grid.yaml` / `dual_add.yaml`）：

```text
config/strategies/bpc/
  meta.yaml                 # 稳定：周期、side、策略身份
  features.yaml             # 稳定：特征管线入口
  research/                 # 可调：训练目标、sweep、研究专用 yaml、README
    # 每节奏「单文件入口」（推荐）：同一 yaml 内分块，见 §3.3「合并 schema」
    turbo.yaml                # pipeline（rolling/history_dir）+ 可选 study + 可选 threshold_search
    slow.yaml                 # 同上；无慢管线时可缺省，解析跳过即可
    # 若希望 PR / 责任边界拆文件，仍允许与 turbo/slow 并列（变体 A，§3.3）
    # study.yaml
    # threshold_search.yaml   # 或 threshold_search.turbo.yaml / threshold_search.slow.yaml
    ...
  archetypes/               # 采纳层：prefilter/gate/execution/direction 等「旋钮」
    prefilter.yaml
    gate.yaml
    ...
  # （可选）根上仍保留一份「全量默认引擎」或完全迁到 archetypes —— 见 §4 merge 约定
```
...
| 优点 | 说明 |
|------|------|
| **所有权清晰** | 「与 bpc 相关、只给 bpc 跑 rolling 的编排」与研究声明、搜索空间 **一并落在 `research/`**（推荐合并进 `turbo.yaml`/`slow.yaml` 分块，见 §3.3），新人路径短。 |
...
| **命名** | 可保留长文件名便于 `rg`；并与 **§3.2 约定名**（`turbo.yaml` / `slow.yaml`）并存：长名做「内容真源」、约定名做「默认入口」或符号链接（二选一在 review 定稿）。 |

**命令示例（迁移后）**：

```bash
BPC_PIPE=config/strategies/bpc/research/turbo.yaml
...
| 路径（相对仓库根） | 含义 |
|--------------------|------|
| `config/strategies/<slug>/research/turbo.yaml` | **快速节奏入口**：`pipeline` 段与当前 `rolling.mode: turbo_fixed_features` 类 prod_train 对齐；可同文件含 `study` / `threshold_search`（§3.3 变体 B）。 |
| `config/strategies/<slug>/research/slow.yaml` | **慢节奏入口**：`pipeline` 段与 `slow_realistic` 类 prod_train 对齐；可选同文件 `study` / `threshold_search`。 |
| `config/strategies/<slug>/research/non_rolling.yaml` | **一次性划分入口（simple）**：仅用顶层 `dates` + `validation_months` 的静态 Train \| Val \| Test 合同；用于 idea / 回归 / 定稿小窗；**不**替代 turbo/slow 主路径。详见 **§14**。 |

可选扩展（review 时取舍）：`research/pipeline.yaml` 作为 **默认**（例如指向与 `turbo.yaml` 相同内容或 `!!include`）；`research/experiment_<name>.yaml` 存放一次性分支。

**解析优先级（建议）**

1. 若用户传入 **`--config <path>`** 且文件存在 → **始终使用该文件**（显式优先）。
2. 若未传 `--config` 但传了 **`--strategy <slug>`** → 依次探测：  
   `research/turbo.yaml` → `research/slow.yaml` →（可选）`research/pipeline.yaml`；**均不存在**则回退到仓库既有默认（如 `config/research_pipeline.yaml`），并 **打印一行警告**，避免 silent 用错 `history_dir`。
3. 若未来支持 **短写** `--config turbo`（仅策略上下文）→ 解析为 `config/strategies/<slug>/research/turbo.yaml`，与显式路径二选一；**禁止**与无 `--strategy` 时组合，避免歧义。
4. （待实现，见 **§14**）若存在 **`research/non_rolling.yaml`** 且用户显式 `--config` 指向该文件 → 走 **静态划分** 管线；**不**参与 turbo/slow 的默认探测顺序，避免 silent 切换 `history_dir`。

**与长文件名并存的方式（二选一）**

- **A（推荐）**：`turbo.yaml` 为真源，长名文件删除或仅在 README 历史表中保留；**或**长名文件保留 git 历史、工作区只用 `turbo.yaml`。
- **B**：`research/turbo.yaml` / `research/slow.yaml` 为 **真源**（整份管线 YAML）；历史 `config/prod_train_*_only.yaml` 已删除或仅保留非 BPC/ME/TPC/chop/dual 主线文件。

**命令示例（约定模式，实现后）**：

```bash
# 等价意图：使用 config/strategies/bpc/research/turbo.yaml（若已实现 §3.2 解析）
...
```bash
# 与「产出根目录」绑定的 yaml 必须与 list/adopt 使用同一份编排（显式路径 或 §3.2 约定 turbo.yaml）
mlbot pipeline run --strategy bpc \
  --config config/strategies/bpc/research/turbo.yaml \
  --stage rolling_sim --skip-shap

# 实现 §3.2 后，可与上式等价（当存在 research/turbo.yaml 且未传 --config 时由 CLI 解析）
# mlbot pipeline run --strategy bpc --stage rolling_sim --skip-shap
```
...
2b. **pipeline 共位（BPC/ME/TPC/多腿已落地）**：turbo/slow 真源为 `config/strategies/<slug>/research/turbo.yaml` 与 `slow.yaml`；`load_pipeline_config` 支持顶层 ``extends`` 链（供其他实验 yaml 复用）。
...
- [ ] **约定名解析**：`mlbot pipeline` 对「仅 `--strategy`」、`research/turbo.yaml` / `slow.yaml` 探测顺序、与 `config/research_pipeline.yaml` 回退的日志与测试是否完备。
- [ ] **研究 yaml 迁入**：根目录 `research.yaml` / `threshold_search.yaml` 迁入 `research/` 后，采用 **§3.3 变体 B（合并入 `turbo.yaml`/`slow.yaml` 的 `pipeline`/`study`/`threshold_search`）** 或 **变体 A（并列文件 + include）** 之一并写死；全仓引用与 loader 一致。
...
## 10. 附录：多腿与经典「信号层」及 turbo/slow 对齐（范式草案）

**动机**：多腿当前以 **regime + 网格/双腿逻辑 + 独立 backtest** 为主，与经典链 **Prefilter → Gate → EntryFilter → Execution** 及 **turbo/slow** 分工 **看起来不统一**；若一开始未明确要求「同一范式」，后续运维、CI、文档和 **adopt/deploy 心智** 都会分叉。**对齐是合理的**。

**原则**：优先 **宏观流程一致**（特征筛选 → 阈值/旋钮 → 分层决策 → 执行参数）与 **落点一致**（`results/` → adopt → `archetypes/` → deploy、`research/turbo|slow`）。**各层 yaml 可用策略贴切命名**（如 Regime 对应经典 Prefilter **那一环的功能**）；须在 **`README.md` 或 `archetypes/README.md`** 写明 **文件名 ↔ pipeline 序位 / profile**，避免 adopt 后无人知晓映射。

### 10.1 两档对齐（建议采纳顺序）

| 档位 | 做什么 | 风险 |
|------|--------|------|
| **A. 流程 / 落点对齐** | 多腿走 **同一套宏观 stage 顺序**（名称可与经典一致：`prefilter` / `gate` / `entry_filter` / `execution`；对内可为 no-op）。rolling、`list/adopt`、`research/turbo|slow` 与经典一致；**采纳结果落在 `archetypes/`**（及约定根 yaml），deploy 一致。**各 stage 读哪份文件** 由 **策略 profile** 声明（允许 `regime.yaml` 等自定义名）。 | 需 **profile 映射** 与 **`enabled: false`**，避免空跑。 |
...
| **入场 refine** | 段内再确认、排除盒等 | **`entry_filters.yaml`** 或暂在根 `grid.yaml`/`dual_add.yaml`。 |
...
### 10.3 turbo / slow 与「挑特征 + 特征阈值 + 执行搜索」

| 节奏 | 经典含义（概括） | 多腿对齐方式 |
|------|------------------|--------------|
| **turbo** | 特征集 **相对固定**，主打 **阈值链 + execution 网格**，月度滚动快 | 多腿已有 `turbo_fixed_features` 类 prod_train：**对齐同一 `research/turbo.yaml` 入口**；特征侧以 **`features.yaml` + feature_dependencies** 定死为主，**threshold_search** 映射到「regime/网格阈值 sweep」而非强行 Pool-B。 |
| **slow** | 结构快照 + 元算法 + 更保守的阈值/EF | 多腿 **`research/slow.yaml`** 存在时：拉长标定窗、打开/加强 **结构类** 校验（若将来与 chop 结构特征挂钩）；**无 slow 文件则跳过**，不要求与 BPC 同月数。 |

**结论**：多腿 **不必** 在第一版就具备与 BPC 完全同构的「特征搜索 + 全量阈值」；但 **应在同一套 turbo/slow 命名与 pipeline 宏观序位下**，把「已有能力」挂上去（**层可自建名**，见 §10.2），并把缺口标为 **roadmap**——与经典在 **流程与落点** 上对齐，在 **术语与文件命名** 上允许诚实差异。

### 10.4 与本文档其它章节的关系

- §3：研究声明与搜索空间 **默认并入** `research/turbo.yaml`、`slow.yaml` 的 **`study` / `threshold_search` 块**（变体 B）；若拆文件（变体 A）则仍可与 **turbo 阶段 sweep** 绑定；**slow** 可无 `threshold_search` 块直至需要。  
- §7 迁移：「范式对齐」可单独开 **Phase：多腿 pipeline 壳 + archetype 落点与 stage profile（含自定义文件名映射）**，与目录搬迁可并行但 **不同 PR** 更易 review。

---

## 11. 多腿 turbo/slow 与「跨 history 列举」：事实核对与产品缺口（共识）

本节回应：**(A)** 多腿是否必须同时有「快管线 + 慢管线」叙事；**(B)** 两份 `prod_train_pipeline_2h_turbo_*_only.yaml` 是否已覆盖「阈值/执行网格、回测报告、交易地图」；**(C)** `mlbot pipeline list` 只能看到「当前配置下的 history」的问题，以及 **列举全部管线历史 + 网站化 adopt/deploy** 的方向。
...
操作员已习惯 **同一 `--config` 下 list/adopt**；§11.1 的缺口应在 **`A快速启动命令.md` 或部署 runbook** 中用一句话强调：**列举 chop/dual 的 turbo 历史时必须指向对应 `prod_train_pipeline_2h_turbo_*_only.yaml`**，直至 §11.4 的跨根命令落地。
...
|------|-----------------------------------------------|
| **同步到 `live/highcap/...` 的内容** | **`archetypes/` 下全部 yaml** + **`grid.yaml` / `dual_add.yaml`** + `TOP_LEVEL_CONFIGS` 中 **实际存在** 的项（`meta.yaml`、`features.yaml` 等）。见 `scripts/deploy_config_to_live.py` 中 `TOP_LEVEL_CONFIGS`、`MULTI_LEG_*` 分支。 |
...
| **仍「混」的点（要优先改的）** | 研究声明、搜索空间 **仍躺在策略根目录**（与 `grid.yaml` 同级），与 **「git 长期意图 / 可上线旋钮」** 同一文件夹，review 与新人导航成本高；与 §2「`research/` 可调、不进 live」的 **物理目录** 尚未完全落地（`chop_grid` 仅有 `research/README.md` 等空壳时，根上 `research.yaml` 仍未迁入 `research/`）。 |

**因此本 ADR 将下列事项标为「稳定流水线最高优先级」（多腿先行亦可）**：

- [ ] **物理拆分**：把 **`research.yaml`、`threshold_search.yaml`**（及未来 **`research/turbo.yaml`** 等）迁入 **`config/strategies/<slug>/research/`**；策略根只保留 **引擎真源**（`grid.yaml` / `dual_add.yaml`）、`meta.yaml`、`features.yaml`、`archetypes/` 等 deploy 白名单内文件。  
...
- [ ] **§3.3 定稿**：`turbo.yaml` / `slow.yaml` 采用 **变体 B（`pipeline`/`study`/`threshold_search` 分块）** 或 **变体 A（并列文件 + include）** 之一；**单一 loader** 解析，禁止「同 repo 两套解析逻辑分叉」。
...
- [ ] **特征与搜索（你关心的「仍能挑选」）**  
  - [ ] **列演进**：`features.yaml` 变更走 **feature_dependencies** 与 review；turbo 默认 **`disable_feature_search: true`** 时，明确 **「谁负责提议新列」**（离线 Pool-B/诊断 vs rolling 内开关）。  
  - [ ] **阈值 / 超参搜索**：`threshold_search`（或并入 `turbo.yaml` 的块）中声明 **regime、网格 spacing、TP、腿数上限** 等搜索空间；**slow** 上可拉长窗或收紧 KPI，与 §10.3 一致。  
...
| ID | 事项 | 详见 |
|----|------|------|
| T01 | 多腿：`research.yaml` / `threshold_search.yaml`（及后续 `research/turbo.yaml` 等）**迁入** `config/strategies/<slug>/research/`；根目录仅保留引擎 yaml + deploy 白名单 | §12.0 |
...
- [ ] Runbook：`mlbot pipeline --config .../non_rolling.yaml` 的 stage 范围与 turbo/slow **对照表**。
