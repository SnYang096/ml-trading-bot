...
- shared trunk：共享特征表示
- heads：每个 head 对应一个 TaskSpec（sr_reversal / breakout / trend…）
- router/gate：按场景选择（可选）

执行层（策略）只需要决定“用哪个 head 的输出去交易”：
- SR reversal strategy → 用 `head=sr_reversal_rr_v1` 的预测
- trend strategy → 用 `head=trend_follow_vX` 的预测
...
`near_sr / compression_score / trend_r2_20` 既可以作为规则层 gating，也应该作为 Router 的输入（regime variables）：

- **作为输入**：让模型学习条件函数 \(y=f(x \mid regime)\)，不同 regime 下对同一语义特征的响应可以不同
- **作为评估切片**：强制输出 `near_sr__* / trend_high__* / compression_high__*` 的 conditional metrics，防止“全局好看但关键 slice 很烂”
- **作为样本加权依据**：让关键 slice 的梯度更大（否则样本占比小，模型容易学成中庸）
