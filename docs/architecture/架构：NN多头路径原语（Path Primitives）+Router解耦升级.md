...
| 维度 | 树模型架构 | 分层架构 | 解决的问题 |
|------|-----------|---------|-----------|
| **特征选择** | 策略特定的Pool B和Semantic Groups | 统一的Feature Layer | 特征复用、减少维护成本 |
| **标签计算** | 策略特定的标签（R/R、三元、百分位） | 统一的路径原语（dir/mfe/mae/mtt） | 策略解耦、可复用、可诊断 |
| **训练数据** | 每个策略独立训练 | 统一的NN模型训练 | 减少训练成本、统一数据质量 |
| **执行策略** | 每个策略独立执行逻辑 | 统一的4个Archetype（TC/TE/FR/ET） | 统一管理、易于监控、风险可控 |
