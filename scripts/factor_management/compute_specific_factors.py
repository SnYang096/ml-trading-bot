...
def compute_factors(
    ...
    df_features = engineer.engineer_all_features(
        input_data, fit=fit, required_features=factors_set
    )

    # 只保留需要的因子列和数据列
    data_cols = {"open", "high", "low", "close", "volume", "timestamp", "datetime"}
    cols_to_keep = [
        col for col in df_features.columns if col in factors_set or col in data_cols
    ]
...
def main():
    ...
    parser.add_argument(
        "--factors", nargs="+", required=True, help="要计算的因子名称列表"
    )
    parser.add_argument("--input", type=str, help="输入文件路径（parquet 或 csv）")
    parser.add_argument(
        "--data-path", type=str, help="数据目录路径（如果使用 MarketDataLoader）"
    )
    parser.add_argument(
        "--symbol", type=str, help="交易对符号（如果使用 MarketDataLoader）"
    )
    parser.add_argument(
        "--start-date", type=str, help="开始日期（如果使用 MarketDataLoader）"
    )
    parser.add_argument(
        "--end-date", type=str, help="结束日期（如果使用 MarketDataLoader）"
    )
    parser.add_argument("--output", type=str, required=True, help="输出文件路径")
    parser.add_argument(
        "--feature-type", type=str, default="comprehensive", help="特征类型"
    )
    parser.add_argument(
        "--format",
        type=str,
