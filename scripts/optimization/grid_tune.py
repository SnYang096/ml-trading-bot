...
def prepare_ohlc(zip_path: str) -> pd.DataFrame:
    ...
    per_sec.columns = [
        "open", "high", "low", "close", "volume", "is_buyer_maker"
    ]
...
def run_once(strategy, feature_engineer, mtf, timeframe, sl, tp, sig_th,
             min_gap_bars):
    ...
    for ts, row in sig.iterrows():
        ...
        for pos in positions:
            ...
            pos["pnl"] = pnl
            if pos["side"] == "long" and (price <= pos["entry"] *
                                          (1 - sl) or price >= pos["entry"] *
                                          (1 + tp)):
                trades.append({**pos, "exit": price, "exit_ts": ts})
                capital += pnl
                pos["status"] = "closed"
            if pos["side"] == "short" and (price >= pos["entry"] *
                                           (1 + sl) or price <= pos["entry"] *
                                           (1 - tp)):
                trades.append({**pos, "exit": price, "exit_ts": ts})
                capital += pnl
        ...
        if row["d"] != 0 and row["strength"] > sig_th:
            if last_sig_ts is None or (
                    ts - last_sig_ts).total_seconds() / 60.0 >= (min_gap_bars *
                                                                 minutes):
                size = (capital * (0.1 if timeframe == "5T" else 0.15) /
                        price) * row["strength"]
                positions.append({
    ...
    pf = ((sum(t.get("pnl", 0) for t in wins) /
           abs(sum(t.get("pnl", 0)
                   for t in losses)) if losses else float("inf"))
          if trades else 0.0)
    return {
        "trades": len(trades),
...
def main():
    ...
    gap_grid = [3, 5]
    results = []
    for tf, sl, tp, sg, gap in product(timeframes, sl_grid, tp_grid, sig_grid,
                                       gap_grid):
    ...
    df = pd.DataFrame(results).sort_values(
        ["timeframe", "return_pct", "profit_factor"],
