"""Run trend_scalp (dual_add) backtest across config/market_segment.yaml windows.

Aggregates per-segment ``summary.csv`` into ``segment_summary.csv`` under --out-root.

Example::

    python scripts/experiment_trend_scalp_market_segment.py \\
      --out-root results/trend_scalp/experiments/segment_validate_20260602 \\
      -- \\
...
def _run_segment(
    *,
    ...
    trades = pd.read_csv(out_dir / "dual_add_trades.csv")
...
def main() -> None:
    ...
    ap.add_argument(
        "--out-root",
        default="results/trend_scalp/experiments/segment_validate",
        help="Parent directory for per-segment outputs",
    )
    ap.add_argument(
        "--market-segment-path",
    ...
    ap.add_argument(
        "--segments",
        default="",
        help="Comma-separated segment ids (default: all in yaml)",
    )
    ...
    for seg in segments:
        ...
        rows.append(row)
        trades = pd.read_csv(seg_dir / "dual_add_trades.csv")
