...
def generate_top_factors_from_grid_search(grid_search_dir: str) -> None:
    ...
    if not selected_features:
        print(
            "❌ Could not find selected_features in best_combination_summary.json or selected_features.txt"
        )
        return
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print(
            "Usage: python generate_top_factors_from_grid_search.py <grid_search_dir>"
        )
        print("\nExample:")
