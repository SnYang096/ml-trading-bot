#!/usr/bin/env python3
"""
Config-driven dimensionality comparison and feature selection.
...
def run_dim_compare(
    ...
    """Run dimensionality comparison for a strategy config."""
    ...
    model_all = train_lightgbm_model(X_train_all, y_train, X_val_all, y_val, keep_all)
    perf_all = evaluate_model(model_all, X_test_all, y_test)

    print("   [After Reduction] Training with representative features...")
    model_reps = train_lightgbm_model(X_train_reps, y_train, X_val_reps, y_val, reps)
    perf_reps = evaluate_model(model_reps, X_test_reps, y_test)
