...
def detect_trading_signals(df: pd.DataFrame, price_col: str,
                           feature_cols: List[str],
                           feature_type: str) -> Tuple[pd.Series, pd.Series]:
    """检测买卖信号
    ...
    if feature_type == 'hurst':
        ...
        for col in feature_cols:
            ...
            buy = (ma_short > ma_long) & (hurst < 0.4) & (hurst.shift(1)
                                                          <= hurst)
            # 卖出信号：短期Hurst下穿长期，且当前值较高
            sell = (ma_short < ma_long) & (hurst > 0.6) & (hurst.shift(1)
                                                           >= hurst)
        ...
        for col in feature_cols:
            ...
            # 买入：相位大幅上升（趋势转折向上）
            buy = (phase_diff > phase_diff.rolling(10).quantile(0.8)) & (phase
                                                                         > 0)
            # 卖出：相位大幅下降（趋势转折向下）
            sell = (phase_diff < phase_diff.rolling(10).quantile(0.2)) & (phase
                                                                          < 0)
    ...
    if len(buy_indices) > len(df) * 0.1:  # 如果信号超过10%的点
        ...
        buy_indices = buy_price_changes.nlargest(int(len(df) *
                                                     0.05)).index  # 只保留前5%
        buy_signals = pd.Series(False, index=df.index)
...
def extract_feature_columns(df: pd.DataFrame,
                            feature_type: str) -> Dict[str, List[str]]:
    """提取指定类型的特征列名
    ...
    for source in signal_sources:
        ...
        if feature_type == 'hurst':
            ...
            spectral_cols = [
                col for col in df.columns
                if col.startswith(f'{source}_spectral')
            ]
...
def create_feature_chart(df: pd.DataFrame,
                         price_col: str,
                         feature_cols: List[str],
                         feature_type: str,
                         source_name: str,
    ...
    buy_signals, sell_signals = detect_trading_signals(df, price_col,
                                                       feature_cols,
                                                       feature_type)
    ...
    if buy_signals_plot.any():
        ...
        fig.add_trace(go.Scatter(
            ...
            showlegend=True,
            hovertemplate=
            '<b>买入信号</b><br>时间: %{x}<br>价格: %{y:.2f}<extra></extra>'),
                      row=1,
                      col=1)
    ...
    if sell_signals_plot.any():
        ...
        fig.add_trace(go.Scatter(
            ...
            showlegend=True,
            hovertemplate=
            '<b>卖出信号</b><br>时间: %{x}<br>价格: %{y:.2f}<extra></extra>'),
                      row=1,
                      col=1)
    ...
    for idx, col in enumerate(feature_cols):
        ...
        # 清理数据：移除 NaN 和 Inf
        y_data = df_plot[col].replace([np.inf, -np.inf],
...
def generate_html_report(
    ...
    for feature_type in feature_types:
        ...
        for source_name, cols in feature_columns.items():
            ...
            print(
                f"      Creating chart for {source_name} ({len(cols)} features)..."
            )
            fig = create_feature_chart(df, price_col, cols, feature_type,
                                       source_name)

            # 将图表转换为 HTML
            chart_html = fig.to_html(include_plotlyjs='cdn',
                                     div_id=f"{feature_type}_{source_name}")
            charts_html.append({
                'type': feature_type,
                'source': source_name,
                'html': chart_html,
...
def main():
    parser = argparse.ArgumentParser(
        description=
        "Visualize hurst, hilbert, wavelet, spectral features as indicators in HTML"
    )
    ...
    parser.add_argument(
        ...
        default=None,
        help=
        "Output HTML file path (default: results/feature_indicators/{symbol}_{timeframe}.html)"
    )
    parser.add_argument(
        "--feature-type",
    ...
    df = loader.load_data(symbol=args.symbol,
    ...
    # 生成 HTML 报告
    generate_html_report(df_features, output_path, args.symbol, feature_types,
                         args.timeframe)
