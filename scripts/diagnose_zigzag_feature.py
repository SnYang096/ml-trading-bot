...
def calculate_zigzag_ic(df: pd.DataFrame, target_col: str = "binary_signal_1") -> dict:
    ...
    for col in ["zigzag", "zigzag_normalized"]:
        if col in df.columns:
            try:
                # 对齐索引
                aligned = pd.DataFrame({"feature": df[col], "target": y}).dropna()
...
def main():
    ...
    try:
        ...
        print(
            f"      - zigzag_normalized 在特征列表中: {feature_check['zigzag_normalized_in_features']}"
        )
    ...
    if zigzag_check.get("zigzag_exists") and not zigzag_check.get(
        "zigzag_normalized_exists"
    ):
        reasons.append("⚠️  zigzag 特征存在但没有归一化版本（zigzag_normalized）")
        reasons.append("   这可能导致 zigzag 因为量纲问题被排除或 IC 值低")

    if feature_check.get("zigzag_in_features") is False:
