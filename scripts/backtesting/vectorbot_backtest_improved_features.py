"""VectorBot backtest with improved feature engineering and normalization."""
...
class ImprovedVectorBotBacktest:
    """VectorBot backtest with improved feature engineering."""

    def __init__(
        self, model_path: str, scaler_path: str, initial_capital: float = 100000
    ):
        """
        Initialize VectorBot backtest with improved features.
        ...
        # Signal parameters
        self.signal_threshold = 0.05  # Lower threshold for signal strength
        self.min_signal_gap = 5  # Minimum bars between signals
    ...
    def run_backtest(self, start_date: str = None, end_date: str = None):
        """Run the backtest with improved features."""
        print("🚀 Starting Improved VectorBot Backtest (with Normalization)")
        print("=" * 60)
        ...
        print(f"Features: {len(feature_columns)} normalized features")
        ...
        # Convert to discrete signals with more flexible thresholds
        signals["discrete_signal"] = 0
        signals.loc[stage1_preds > 0.55, "discrete_signal"] = (
            1  # Long (lowered threshold)
        )
        signals.loc[stage1_preds < 0.45, "discrete_signal"] = (
            -1
        )  # Short (raised threshold)
    ...
    def save_results(self):
        ...
        trades_df.to_csv("improved_features_vectorbot_trades.csv", index=False)

        # Save equity curve
        equity_df = pd.DataFrame(self.equity_curve)
        equity_df.to_csv("improved_features_vectorbot_equity_curve.csv", index=False)

        # Save results
        with open("improved_features_vectorbot_results.json", "w") as f:
            json.dump(self.results, f, indent=2)

        print(f"\n💾 Results saved:")
        print(f"   - improved_features_vectorbot_trades.csv")
        print(f"   - improved_features_vectorbot_equity_curve.csv")
        print(f"   - improved_features_vectorbot_results.json")


def main():
    """Main function to run improved VectorBot backtest."""
    ...
    # Initialize backtest
    backtest = ImprovedVectorBotBacktest(
        model_path, scaler_path, initial_capital=100000
    )
