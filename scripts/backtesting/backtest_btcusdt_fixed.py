"""Fixed backtest script for BTCUSDT data using the ML trading strategy."""
...
def run_backtest():
    ...
    print("🚀 BTCUSDT ML Trading Strategy Backtest (Fixed)")
    print("=" * 50)
    ...
    try:
        ...
        # Create signals DataFrame with proper price data
        ...
        # Add price information for analysis - use the same index
        ...
        signals = signals.merge(
            price_data, left_index=True, right_index=True, how="left"
        )

        # Save signals
        signals.to_csv("btcusdt_backtest_signals_fixed.csv", index=False)
        print(f"\n   ✓ Saved signals to btcusdt_backtest_signals_fixed.csv")
        ...
        # Show sample signals with price data
        print(f"\n   Sample signals with price data:")
        sample_cols = [
            "timestamp",
            "close",
            "stage1_pred",
        ...
        available_cols = [col for col in sample_cols if col in signals.columns]
        print(signals[available_cols].head(10))

        print("\n🎉 Backtest completed successfully!")
        print("\nNext steps:")
        print("1. Check btcusdt_backtest_signals_fixed.csv for the generated signals")
        print("2. Run python view_btcusdt_signals_fixed.py to analyze the results")
