...
print("Loading trained May wavelet model...")
with open(MODEL_PATH, "rb") as f:
    model_data = pickle.load(f)
strategy = model_data["strategy"]
feature_engineer = model_data["feature_engineer"]
print("Loading scalers (no refit)...")
feature_engineer.load_scalers(SCALER_PATH)

print("Extracting June zip and preparing OHLCV...")
temp_dir = os.path.join("data", "temp_extract_june")
os.makedirs(temp_dir, exist_ok=True)
with zipfile.ZipFile(JUNE_ZIP, "r") as z:
    z.extractall(temp_dir)
csv_files = [f for f in os.listdir(temp_dir) if f.endswith(".csv")]
if not csv_files:
    raise SystemExit("No CSV found in June zip")
...
print(f"June 1s bars: {len(ohlc)}, range: {ohlc.index[0]} -> {ohlc.index[-1]}")
...
def run_bt(
    ...
    for ts, row in sig.iterrows():
        ...
        for pos in positions:
            ...
            reached_2r = (
                (price >= pos["entry"] * (1.0 + 2 * take_profit_pct))
                if pos["side"] == "long"
                else (price <= pos["entry"] * (1.0 - 2 * take_profit_pct))
            )
            if (
                pos["status"] == "active"
                and not pos.get("tp1_done", False)
                and reached_1r
            ):
                close_size = pos["size"] * 0.5
                close_pnl = (
                    (price - pos["entry"]) * close_size
                    if pos["side"] == "long"
                    else (pos["entry"] - price) * close_size
                )
                trades.append(
                    {
                        **pos,
            ...
            atr = float(data_tf.loc[ts, "atr"]) if "atr" in data_tf.columns else None
            if (
                pos["status"] == "active"
                ...
                if pos["side"] == "long":
                    pos["stop"] = max(pos["stop"], price - atr_trail_mult * atr)
                else:
                    pos["stop"] = min(pos["stop"], price + atr_trail_mult * atr)
            # stop check
            if pos["side"] == "long" and price <= pos["stop"]:
                close_pnl = (price - pos["entry"]) * pos["size"]
                trades.append({**pos, "exit": price, "exit_ts": ts, "reason": "stop"})
                capital += close_pnl
                pos["status"] = "closed"
            ...
            if pos["status"] == "active" and reached_2r:
                ...
                trades.append(
                    {**pos, "exit": price, "exit_ts": ts, "reason": "tp2_full"}
                )
                capital += close_pnl
                pos["status"] = "closed"
            ...
            adverse = (
                ...
                if pos["side"] == "long"
                else (price >= pos["entry"] * (1.0 + 0.5 * stop_loss_pct))
            )
            if (
                pos["status"] == "active"
                and adverse
        ...
        if row["d"] != 0 and row["strength"] > signal_threshold:
            ...
            if (
                last_sig_ts is None
                or (ts - last_sig_ts).total_seconds() / 60.0 >= (min_gap_bars * minutes)
                ...
                stop_price = (
                    ...
                    if (row["d"] == 1 and atr and atr > 0)
                    else (entry * (1.0 - stop_loss_pct) if row["d"] == 1 else None)
                )
                if row["d"] != 1:
                    stop_price = (
                        (entry + atr_stop_k * atr)
                ...
                raw_size = (capital * risk_pct) / stop_dist if stop_dist > 0 else 0.0
                ...
                # leverage cap
                exposure = sum(
                    [(p["size"] * price) for p in positions if p["status"] == "active"]
                )
        ...
        for pos in positions:
            if pos["status"] != "active":
                continue
            progressed_05r = (
                (price >= pos["entry"] * (1.0 + 0.5 * take_profit_pct))
                if pos["side"] == "long"
                else (price <= pos["entry"] * (1.0 - 0.5 * take_profit_pct))
            )
            progressed_10r = (
                (price >= pos["entry"] * (1.0 + 1.0 * take_profit_pct))
                if pos["side"] == "long"
                else (price <= pos["entry"] * (1.0 - 1.0 * take_profit_pct))
            )
            if pos["adds"] < max_adds and (progressed_05r or progressed_10r):
                ...
                add_raw = (
                    (capital * (risk_pct * add_risk_frac)) / stop_dist_now
                    if stop_dist_now > 0
                    else 0.0
                )
                # leverage cap on add
                ...
                remain_notional = max(0.0, max_notional - exposure)
                add_size = (
                    min(add_raw, remain_notional / price)
                    if remain_notional > 0
                    else 0.0
                )
                if add_size > 0:
        ...
        eq = capital + open_pnl
        equity.append(
            {"timestamp": ts, "equity": eq, "capital": capital, "open_pnl": open_pnl}
        )
        if eq > peak_eq:
            peak_eq = eq
    ...
    res = {
        ...
        "avg_win": float(avg_win) if wins else 0.0,
        "avg_loss": float(avg_loss) if losses else 0.0,
        "profit_factor": (
            (
                abs(avg_win * len(wins) / (avg_loss * len(losses)))
                if losses
                else float("inf")
            )
            if trades
    ...
    pd.DataFrame(trades).to_csv(
        os.path.join(RESULTS_DIR, f"wavelet_{timeframe}_june_trades.csv"), index=False
    )
    pd.DataFrame(equity).to_csv(
        os.path.join(RESULTS_DIR, f"wavelet_{timeframe}_june_equity_curve.csv"),
        index=False,
    )
    with open(
        os.path.join(RESULTS_DIR, f"wavelet_{timeframe}_june_results.json"), "w"
    ) as f:
