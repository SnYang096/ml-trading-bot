"""VectorBot backtest with wavelet transform features."""
...
class WaveletVectorBotBacktest:
    """VectorBot backtest with wavelet transform features."""
    ...
    def __init__(
        ...
        """
        Initialize VectorBot backtest with wavelet features.
    ...
    def load_model(self):
        ...
        print(f"Loading wavelet model from {self.model_path}...")
        ...
        print("✅ Wavelet model loaded successfully")
    ...
    def run_backtest(self, start_date: str = None, end_date: str = None):
        """Run the backtest with wavelet features."""
        print("🚀 Starting Wavelet VectorBot Backtest")
        print("=" * 60)
        ...
        print("\n🎉 Wavelet backtest completed successfully!")
    ...
    def save_results(self):
        ...
        trades_df.to_csv("wavelet_vectorbot_trades.csv", index=False)

        # Save equity curve
        equity_df = pd.DataFrame(self.equity_curve)
        equity_df.to_csv("wavelet_vectorbot_equity_curve.csv", index=False)

        # Save results
        with open("wavelet_vectorbot_results.json", "w") as f:
            json.dump(self.results, f, indent=2)

        print(f"\n💾 Results saved:")
        print(f"   - wavelet_vectorbot_trades.csv")
        print(f"   - wavelet_vectorbot_equity_curve.csv")
        print(f"   - wavelet_vectorbot_results.json")


def main():
    """Main function to run wavelet VectorBot backtest."""
    ...
    # Initialize backtest
    backtest = WaveletVectorBotBacktest(model_path, scaler_path, initial_capital=100000)
