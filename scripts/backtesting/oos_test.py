"""OOS test script - supports any trained model and test data."""
...
def main():
    parser = argparse.ArgumentParser(description="OOS test with trained model")
    parser.add_argument(
        "--model", type=str, required=True, help="Model name (without extension)"
    )
    parser.add_argument(
        "--data", type=str, required=True, help="Path to test data zip file"
    )
    parser.add_argument(
        "--output", type=str, default="oos_results", help="Output directory name"
    )

    args = parser.parse_args()

    print("\n" + "=" * 70)
    print("📊 OOS Test: Out-of-Sample Evaluation")
    print("=" * 70)
    print(f"\n📋 Configuration:")
    ...
    if not os.path.exists(model_path):
        ...
        print(
            f"   python scripts/train_model_gpu.py --data <train_data.zip> --model-name {args.model}"
        )
        return

    print(f"\n✅ Test data found ({os.path.getsize(args.data) / (1024**3):.2f} GB)")
    print(f"✅ Model found")
