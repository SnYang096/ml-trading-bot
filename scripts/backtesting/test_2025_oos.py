"""Test 2025 Q1-Q2 as pure OOS (Out-of-Sample) using 2024Q4 model."""
...
def main():
    ...
    print("\n" + "=" * 80)
    print("🚀 2025 Out-of-Sample (OOS) Test")
    print("=" * 80 + "\n")

    # Use 2024Q2 model (latest available) to test 2025
    model_path = os.path.join(model_dir, "model_2024Q2.txt")

    if not os.path.exists(model_path):
        print("❌ Model 2024Q2 not found!")
        return

    print(f"📦 Loading model 2024Q2 (trained on 2021-2024Q2)...")
    model = lgb.Booster(model_file=model_path)

    all_results = []

    # Test on 2025Q1 and 2025Q2
    for q in [1, 2]:
        quarter_str = f"2025Q{q}"
        print(f"\n{'='*80}")
        print(f"Testing on {quarter_str}")
        print(f"{'='*80}\n")

        print(f"📊 Loading test data {quarter_str}...")
        test_df = load_quarter_data(data_dir, 2025, q)

        if test_df is None or len(test_df) == 0:
            print(f"❌ No data for {quarter_str}")
            continue
        ...
        results["model"] = "2024Q2"
        results["test_period"] = quarter_str

        all_results.append(results)

        print(f"\n📈 Results for {quarter_str}:")
    ...
    # Save
    output_dir = "results/quarterly_rolling_btc"
    os.makedirs(output_dir, exist_ok=True)
    output_path = os.path.join(output_dir, "2025_oos_results.csv")
    results_df.to_csv(output_path, index=False)

    print(
        f"{'Period':<12} {'Trades':<8} {'Return':<10} {'Win%':<8} {'MaxDD':<10} {'Sharpe':<8}"
    )
    print("-" * 80)
    for _, row in results_df.iterrows():
        print(
            f"{row['test_period']:<12} {row['total_trades']:<8.0f} "
            f"{row['total_return']:>8.2f}% {row['win_rate']:>6.1f}% "
            f"{row['max_drawdown']:>8.2f}% {row['sharpe_ratio']:>6.2f}"
        )
