...
async def test_binance_mainnet_websocket(
    ...
    """
    # 临时取消代理环境变量，避免影响WebSocket连接
    import os

    original_proxy = os.environ.get("HTTP_PROXY")
    original_https_proxy = os.environ.get("HTTPS_PROXY")
    if original_proxy:
        print(f"⚠️  检测到代理设置: HTTP_PROXY={original_proxy}")
        print(f"   临时取消代理环境变量以测试WebSocket连接...")
    ...
    if len(symbols) == 1:
        # 单个symbol使用单个流格式
        url = f"wss://fstream.binance.com/ws/{symbols[0].lower()}@trade"
    else:
        # 多个symbol使用组合流格式
...
async def main():
    ...
    await test_binance_mainnet_websocket(args.symbols, args.duration)
