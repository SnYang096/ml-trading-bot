...
若要重跑归档管线，使用各目录旁或 `meta.yaml` 中注释的 `prod_train_pipeline_*` 路径（RMR：`config/prod_train_pipeline_2h_slow_rmr_only.yaml`，其中 `strategies.rmr.config` 已指向本目录；EML：`config/prod_train_pipeline_2h_turbo_eml_only.yaml`）。
