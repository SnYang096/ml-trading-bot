...
- `box_breakout_up/down` 硬编码 `box_hi_120` / `box_lo_120`，与 `lookback_breakout=240` 未对齐。
- `B_L120_retest` 仍用 prod soft_phase 的 depth/recovery，非完整「突破 → 回测 box 顶」语义。

**下一步**：完成 Phase 1 扫描 → 更新变体树与 grid → 再跑 Phase 3。
