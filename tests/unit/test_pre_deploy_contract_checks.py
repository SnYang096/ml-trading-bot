...
def test_gate_robustness_default_skips_prefilter(tmp_path: Path) -> None:
    arch = tmp_path / "strategies" / "tpc" / "archetypes"
    arch.mkdir(parents=True)
    ...
    items = summary["strategies"]["tpc"]["checks"]["plateau_stability"][
        "robustness_items"
    ]
    files = {i["file"] for i in items}
    assert "gate.yaml" in files
    assert "prefilter.yaml" not in files
