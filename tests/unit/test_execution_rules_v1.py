...
def test_execution_rules_veto_on_missing_keys() -> None:
    rules = ExecutionRulesV1(
        required_keys_by_archetype={"MomentumExpansion": ["need_a", "need_b"]}
    )
    ok, reasons = apply_execution_rules_v1(
        rules=rules, archetype_name="MomentumExpansion", features={"need_a": 1}
    )
    assert not ok
...
def test_execution_rules_allow_when_present() -> None:
    rules = ExecutionRulesV1(
        required_keys_by_archetype={"MomentumExpansion": ["need_a"]}
    )
    ok, reasons = apply_execution_rules_v1(
        rules=rules, archetype_name="MomentumExpansion", features={"need_a": 1}
    )
